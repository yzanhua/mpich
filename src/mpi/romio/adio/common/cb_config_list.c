...
int ADIOI_cb_config_list_parse(char *config_list,
                               ADIO_cb_name_array array, int ranklist[], int cb_nodes)
{
    ...
    if (cb_nodes > nr_procnames)
        cb_nodes = nr_procnames;
...
int ADIOI_cb_copy_name_array(MPI_Comm comm,
                             int keyval, void *extra, void *attr_in, void **attr_out, int *flag)
{
                       ...
                       int nr_procnames, int ranks[], int nr_ranks, int *nr_ranks_allocated)
{
    ...
    if (name == NULL) {
        ...
        while (nr_ranks - *nr_ranks_allocated > 0) {
            /* find a name */
            while ((wildcard_proc < nr_procnames) && (used_procnames[wildcard_proc] != 0)) {
                wildcard_proc++;
            }
            ...
            *nr_ranks_allocated = *nr_ranks_allocated + 1;
            ...
            ret = match_this_proc(procnames[wildcard_proc], cur_proc,
                                  ...
                                  nr_procnames, ranks, nr_ranks, *nr_ranks_allocated);
            if (ret > 0)
                *nr_ranks_allocated = *nr_ranks_allocated + ret;
                           ...
                           int nr_procnames, int ranks[], int nr_ranks, int nr_ranks_allocated)
{
    ...
    nr_to_alloc = (max_matches < ranks_remaining) ? max_matches : ranks_remaining;

    while (nr_to_alloc > 0) {
        cur_proc = find_name(name, procnames, used_procnames, nr_procnames, cur_proc);
        if (cur_proc < 0) {
            /* didn't find it */
            return nr_ranks_allocated - old_nr_allocated;
        }
...
static int find_name(char *name,
                     char *procnames[], char used_procnames[], int nr_procnames, int start_ind)
{
    int i;

    for (i = start_ind; i < nr_procnames; i++) {
        if (!used_procnames[i] && !strcmp(name, procnames[i]))
            break;
    }

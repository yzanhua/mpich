                                ...
                                ADIO_Offset offset, MPI_Request * request, int *error_code)
{
    ...
    vars =
        (ADIOI_GEN_IreadStridedColl_vars *) ADIOI_Calloc(1,
                                                         sizeof(ADIOI_GEN_IreadStridedColl_vars));
...
static void ADIOI_GEN_IreadStridedColl_inter(ADIOI_NBC_Request * nbc_req, int *error_code)
{
...
static void ADIOI_GEN_IreadStridedColl_indio(ADIOI_NBC_Request * nbc_req, int *error_code)
{
    ADIOI_GEN_IreadStridedColl_vars *vars = nbc_req->data.rd.rsc_vars;
    ...
    if (fd->hints->cb_read == ADIOI_HINT_DISABLE
        ...
        if (vars->buftype_is_contig && filetype_is_contig) {
            if (file_ptr_type == ADIO_EXPLICIT_OFFSET) {
                ...
                ADIO_IreadContig(fd, buf, count, datatype, ADIO_INDIVIDUAL,
...
static void ADIOI_GEN_IreadStridedColl_read(ADIOI_NBC_Request * nbc_req, int *error_code)
{
    ADIOI_GEN_IreadStridedColl_vars *vars = nbc_req->data.rd.rsc_vars;
     ...
     */
    rae_vars = (ADIOI_Iread_and_exch_vars *) ADIOI_Calloc(1, sizeof(ADIOI_Iread_and_exch_vars));
...
static void ADIOI_GEN_IreadStridedColl_free(ADIOI_NBC_Request * nbc_req, int *error_code)
{
    ADIOI_GEN_IreadStridedColl_vars *vars = nbc_req->data.rd.rsc_vars;
    ADIO_File fd = vars->fd;
...
static void ADIOI_GEN_IreadStridedColl_fini(ADIOI_NBC_Request * nbc_req, int *error_code)
{
    ADIOI_GEN_IreadStridedColl_vars *vars = nbc_req->data.rd.rsc_vars;
    MPI_Count size;
...
static void ADIOI_Iread_and_exch_l1_begin(ADIOI_NBC_Request * nbc_req, int *error_code)
{
    ADIOI_Iread_and_exch_vars *vars = nbc_req->data.rd.rae_vars;
    ADIO_File fd;
    int nprocs;
                    ...
                    if (real_off + real_size - req_off < (ADIO_Offset) (unsigned) req_len) {
                        ...
                        if ((j + 1 < others_req[i].count) &&
                            ...
                            for_next_iter = MPL_MAX(for_next_iter,
                                                    real_off + real_size - others_req[i].offsets[j +
                                                                                                 1]);
    ...
    red_vars = (ADIOI_R_Iexchange_data_vars *) ADIOI_Calloc(1, sizeof(ADIOI_R_Iexchange_data_vars));
    ...
    if (flag) {
        ...
        ADIO_ReadContig(fd, read_buf + vars->for_curr_iter, (int) size,
                        MPI_BYTE, ADIO_EXPLICIT_OFFSET, vars->off, &status, error_code);
#else
        ADIO_IreadContig(fd, read_buf + vars->for_curr_iter, (int) size,
                         MPI_BYTE, ADIO_EXPLICIT_OFFSET, vars->off, &vars->req2, error_code);
...
static void ADIOI_Iread_and_exch_l1_end(ADIOI_NBC_Request * nbc_req, int *error_code)
{
    ADIOI_Iread_and_exch_vars *vars = nbc_req->data.rd.rae_vars;
    ADIO_File fd = vars->fd;
...
static void ADIOI_Iread_and_exch_reset(ADIOI_NBC_Request * nbc_req, int *error_code)
{
    ADIOI_Iread_and_exch_vars *vars = nbc_req->data.rd.rae_vars;
    int nprocs = vars->nprocs;
...
static void ADIOI_Iread_and_exch_l2_begin(ADIOI_NBC_Request * nbc_req, int *error_code)
{
    ADIOI_Iread_and_exch_vars *vars = nbc_req->data.rd.rae_vars;
    ADIOI_R_Iexchange_data_vars *red_vars = NULL;
...
static void ADIOI_Iread_and_exch_l2_end(ADIOI_NBC_Request * nbc_req, int *error_code)
{
    ADIOI_Iread_and_exch_vars *vars = nbc_req->data.rd.rae_vars;
...
static void ADIOI_R_Iexchange_data_recv(ADIOI_NBC_Request * nbc_req, int *error_code)
{
    ADIOI_R_Iexchange_data_vars *vars = nbc_req->data.rd.red_vars;
    ADIO_File fd = vars->fd;
    int *send_size = vars->send_size;
